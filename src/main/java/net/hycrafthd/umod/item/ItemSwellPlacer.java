...
/*	@Override
	public ActionResult<ItemStack> onItemRightClick(ItemStack itemStackIn, World world, EntityPlayer playerIn,
		...
		if (start == null)
			return false;
		if (world.getBlockState(start.add(0, -1, 0)).getBlock().isFullBlock(state) && world.getBlockState(start.add(1, -1, 0)).getBlock().isSolidFullCube() && !(world.getBlockState(start.add(1, 0, 0)).getBlock().isSolidFullCube())) {
