...
public class WavefrontVertex {
	...
	public void addVertex(Material mat, RenderBuffer buffer, ArrayList<Vec3d> vertieces, ArrayList<Vec2d> vertex_texture_cords, ArrayList<Vec3d> vertex_normals) {
		if (this.VERTEX <= 0)
			return;
		int ver = buffer.addVertex(vertieces.get(this.VERTEX - 1));
		
		if (this.VERTEX_TEXTURE > 0) {
			Vec2d texture = vertex_texture_cords.get(this.VERTEX_TEXTURE - 1);
			buffer.addUV(texture, ver);
		}
